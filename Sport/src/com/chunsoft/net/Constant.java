package com.chunsoft.net;

public class Constant {
	public static boolean isLogin = false;
	public static String IP = "http://203.110.165.236/";
	public static String IMMEDIATE = "matches/immediate.json?q[match_level_lte]=0";
	public static String LOGIN = "users/sign_in.json";
	public static String REGISTER = "users.json";
	public static String MATCH_RECOMMENDS = "match_recommands.json";
	public static String EVENTS = "my_favorites/{user_id}/events.json";
	public static String RECOMMEND_RESULT = "match_recommands/report";
	public static String ODD_CHANGES = "odd_changes";
	public static String ODD_CHANGES_DATA = "odd_changes.json";

}
